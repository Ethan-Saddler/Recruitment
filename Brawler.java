public class Brawler extends SuperheroRecruit {
    ...
    public Brawler(Brawler hero) {
        super(hero.name, hero.speed, hero.strength, hero.intelligence);
    }
    public double powerScaling() {
        double average = (speed + strength + intelligence) / 3;
        if (catchphrase != null) {
    ...
    public void statChange() {
        System.out.printf("Catchphrase: %s%nOriginal Speed and Strenght: %d and %f%nStat Increase: %f%%nFinal Power Scaling: %d%n", catchphrase.getCatchphrase(), speed, strength, (catchphrase == null ? 0.0 : catchphrase.getStatIncrease()), powerScaling());
    }
